...
/*
Trading strategy used by Richard Dennis and his students
*/
...
extern string  Header6 = "----------Set Max Loss Limit-----------";
extern bool    IsLossLimitActivated = False;
extern double  LossLimitPercent = 0; 

extern string  Header7 = "----------EA General Settings-----------";
...
double turtleATR;
double Close1, DonchianTopEntry1, DonchianBottomEntry1, DonchianTopExit1, DonchianBottomExit1;
...
int start()
  {
      ...
      // Assign values to variables. Declare Cross functions
      
      DonchianTopEntry1 = iCustom(NULL, 0, "Donchian Channels", DonchianPeriodsEntry, MaxBars, 1, 1);
      DonchianBottomEntry1 = iCustom(NULL, 0, "Donchian Channels", DonchianPeriodsEntry, MaxBars, 0, 1);
      ...
      turtleATR = iATR(NULL,Period(),ATRperiod,1);
      
      Stop=VolBasedStopLoss(IsVolatilityStopOn, FixedStopLoss, turtleATR, VolBasedSLMultipler, P);
      Take=VolBasedTakeProfit(IsVolatilityTakeProfitOn, FixedTakeProfit, turtleATR, VolBasedTPMultipler, P);
   
      // Modify Exit and Entry Rules
      ...
      if(IsMaxPositionsReached(MaxPositionsAllowed, MagicNumber, OnJournaling) == False){
         if(entryLock == False) if(EntrySignal(LongEntryBreakout)==1){ // Open Long Positions
            ...
            OpenPositionPending(OP_BUYSTOP, Ask + 0.5*turtleATR, 0, GetLot(IsSizingOn, Lots, Risk, YenPairAdjustFactor, Stop, P), 0, 0, MagicNumber, Slippage, OnJournaling, P, IsECNbroker, MaxRetriesPerTick);
            OpenPositionPending(OP_BUYSTOP, Ask + 1.0*turtleATR, 0, GetLot(IsSizingOn, Lots, Risk, YenPairAdjustFactor, Stop, P), 0, 0, MagicNumber, Slippage, OnJournaling, P, IsECNbroker, MaxRetriesPerTick);
            OpenPositionPending(OP_BUYSTOP, Ask + 1.5*turtleATR, 0, GetLot(IsSizingOn, Lots, Risk, YenPairAdjustFactor, Stop, P), 0, 0, MagicNumber, Slippage, OnJournaling, P, IsECNbroker, MaxRetriesPerTick);
            ...
            OpenPositionPending(OP_SELLSTOP, Bid - 1.5*turtleATR, 0, GetLot(IsSizingOn, Lots, Risk, YenPairAdjustFactor, Stop, P), 0, 0, MagicNumber, Slippage, OnJournaling, P, IsECNbroker, MaxRetriesPerTick);  
      ...
      // Manage Stop Loss modifications and position/order termination
...
Content:
1) EntrySignal (Part of TDL 1)
2) ExitSignal (Part of TDL 1)
3) GetLot
4) CheckLot
5) CountPosOrders
...
17) TurtleModifyStops (TDL 4)
18) HasPositionsDecreased (TDL 4)
19) HasPositionsIncreased (TDL 4)
...
// This function closes positions and deletes orders.
